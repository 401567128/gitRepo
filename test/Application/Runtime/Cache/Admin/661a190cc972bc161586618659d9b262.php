...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a href="javascript:;" onclick="admin_add('添加管理员','admin-add.html','800','500')" class="btn btn-primary radius" id="adminAdd">
				<i class="Hui-iconfont">&#xe600;</i> 
				添加管理员
			</a>
		</span> 
		<span class="r">共有人数：<strong><?php echo ($userCount); ?></strong> 人</span> 
	</div>
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="12">人员列表</th>
			</tr>
			<tr class="text-c">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td><?php echo ($vo["setuptime"]); ?></td>
					<td><?php echo ($vo["lasttime"]); ?></td>
					<td><?php echo ($vo["ip"]); ?></td>
					<td>
						<?php if(handlePms($vo['permissions'],"1")) echo "<span style='color:red'>超级管理员</span></br>" ; if(handlePms($vo['permissions'],"2")) echo "留言管理</br>" ; if(handlePms($vo['permissions'],"3")) echo "基本设置</br>" ; if(handlePms($vo['permissions'],"4")) echo "敏感词汇</br>" ; ?>
					</td>


					<td class="td-status">
					    <?php if($vo["locked"] == 0): ?><span class="label label-success radius">未禁用</span>
					        <?php else: ?> <span class="label label-default radius">已禁用</span><?php endif; ?>
					</td>
					<td class="td-manage">
						<?php if($vo["locked"] == 0): ?><a style="text-decoration:none" onClick='admin_stop(this,"<?php echo ($vo["id"]); ?>")' href="javascript:;" title="停用">
								<i class="Hui-iconfont">&#xe631;</i>
							</a>
					        <?php else: ?>
						...
						<a title="编辑" href="javascript:;" onclick='admin_edit("管理员编辑","admin_edit.html?uid=<?php echo ($vo["id"]); ?>","800","500")' class="ml-5" style="text-decoration:none">
							<i class="Hui-iconfont">&#xe6df;</i>
						</a> 
	...
	*/
	...
	function admin_del(obj,id){
		layer.confirm('确认要删除吗？',function(index){
			...
			$(document).ready(function() {
				$.post("<?php echo U('Index/admin_delete');?>",{uid:id},function(data){
					if(data == 1){
	...
	function admin_stop(obj,id){
		layer.confirm('确认要停用吗？',function(index){
			//此处请求后台程序，下方是成功后的前台处理……
			$(document).ready(function() {
				$.post("<?php echo U('Index/locked');?>",{uid:id,locked:"1"},function(data){
					if(data == 1){
	...
	function admin_start(obj,id){
		layer.confirm('确认要启用吗？',function(index){
			//此处请求后台程序，下方是成功后的前台处理……
			$(document).ready(function() {
				$.post("<?php echo U('Index/locked');?>",{uid:id,locked:"0"},function(data){
					if(data == 1){
